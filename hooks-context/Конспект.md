...
3. Если `callback` возвращает функцию, она вызовется при размонтировании или при изменении зависимостей.
...
export default function LatestNewsStable() {
  ...
  useEffect(() => {
    const id = setInterval(async () => {
      ...
      if (tsRef.current === ts) setNews(data);
    }, 5000);
...
* **Provider** — `<Ctx.Provider value={...}>...</Ctx.Provider>`
  Доступ к данным для всего дерева компонентов.

* **useContext** — `const value = useContext(Ctx)`
  Получение текущего значения контекста.

* **Consumer** — `<Ctx.Consumer>{value => ...}</Ctx.Consumer>`
  Альтернатива `useContext`, работает в классовых компонентах.
